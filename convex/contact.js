...
export const getAllContacts = query({
    handler: async (ctx) => {
        ...
        // Combine expenses paid by you and not paid by you
        const personalExpenses = [...expensesYouPaid, ...expensesNotPaidByYou];
        const contactIds = new Set();
        // Collect all unique userIds involved in these expenses
        personalExpenses.forEach((exp) => {
            // Add the person who paid, if not the current user
            if (exp.paidByUserId !== currentUser._id) {
                contactIds.add(exp.paidByUserId);
            }
            // Add all users in the splits except the current user
        ...
        // Resolve user objects for each contactId
        ...
        //step 9:  return the result filtering out nulls
        return {
